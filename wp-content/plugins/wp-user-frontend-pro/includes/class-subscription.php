    ...
    public function wpuf_send_subs_notification() {
        ...
        foreach ( $users as $user ) {
            ...
            if ( ! empty( $sub ) && ( $sub !== 'Cancel' || $sub !== 'cancel' ) && $enabled_sub_noti === 'on' ) {
                ...
                $end_date      = gmdate( 'Y-m-d h:i:s', strtotime( $start_date . " + {$duration} {$duration_type}" ) );

                $curr_time = date_create( gmdate( 'Y-m-d h:i:s', time() ) );
                $time_diff = date_diff( $curr_time, date_create( $end_date ) );

                if ( ( $curr_time < $end_date ) && ( $time_diff->days <= $date_before ) && get_user_meta( $user->ID, 'wpuf_pre_sub_exp', true ) !== 'sent' ) {
                    $this->wpuf_pre_sub_exp_notification( $user, $sub );
                    update_user_meta( $user->ID, 'wpuf_pre_sub_exp', 'sent' );
                } elseif ( ( $curr_time > $end_date ) && ( $time_diff->days >= $date_after ) && get_user_meta( $user->ID, 'wpuf_post_sub_exp', true ) !== 'sent' ) {
