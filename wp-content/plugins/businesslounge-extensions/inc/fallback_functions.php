...
if( ! function_exists("rtframework_create_carousel") ){
	...
	function rtframework_create_carousel( $contents = array(), $atts = array(), $thumbnails = array() ){
		...
		//create final output
		$output = sprintf('
				<div id="%1$s" class="rt-carousel carousel-holder clearfix %2$s" data-item-width="%4$s" data-nav="%5$s" data-dots="%6$s" data-margin="%8$s" data-autoplay="%9$s" data-timeout="%10$s" data-thumbnails="%12$s" data-boxed="%13$s" data-min-height="%14$s">
...
if( ! function_exists("rtframework_create_photo_gallery") ){
	...
	function rtframework_create_photo_gallery( $atts ){
		...
		$gallery_holder_output = sprintf('
			<div class="photo_gallery rt_lightbox_gallery clearfix %s" id="%s" data-column-width="%s">%s</div> 
...
if( ! function_exists("rtframework_excerpt_more") ){
	/**
	 * Adds read more link end of the excerpts
	 * 
	 * @return void
	 */
	function rtframework_excerpt_more($excerpt) {
		global $post;
		$excerpt = rtrim($excerpt,'[&hellip;]');
		$excerpt = rtrim($excerpt,'[...]');
		$replace = '<a class="more-link" href="'. get_permalink($post->ID) . '">'.esc_html__( 'Continue reading', 'businesslounge' ).'</a>';
		$excerpt .= '... '.$replace;
		return $excerpt;
	}	
}
add_filter('the_excerpt', 'rtframework_excerpt_more',0);
